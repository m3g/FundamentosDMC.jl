...
program md_berendsen
  ...
  write(*,"( 75('-'),/,' MD with Berendsen bath ',/,75('-') )")

  write(*,"( a,$ )") ' Enter bath characteristic time (tau): ' 
  read(*,*) tau
  ...
  do istep = 1, nsteps
    ...
    if ( mod(istep,iprint) == 0 ) then
      ...
      if ( time > tequil .or. printequil ) then
    ...
    if ( istep <= nequil ) then
      ...
      vx = 0.d0
      vy = 0.d0
      lambda = dsqrt( 1 + (dt/tau)*(kavg_target/kavg-1) )
      do i = 1, n
        v(i,1) = v(i,1) * lambda
        v(i,2) = v(i,2) * lambda
        vx = vx + v(i,1)
